...
REM Auto-elevacion: si no tiene admin, se relanza como admin.
REM Usamos el argumento __ADMIN__ como marcador para no loopear.

if "%~1"=="__ADMIN__" goto :main

net session >nul 2>&1
if %errorlevel% equ 0 goto :main

echo  Solicitando permisos de administrador...
powershell -Command "Start-Process -FilePath '%~f0' -ArgumentList '__ADMIN__' -Verb RunAs -WorkingDirectory '%~dp0'"
exit /b

:main

REM Buscar el ejecutable de la app (misma carpeta o subcarpeta dist\)
set "APP="
...
REM Detectar adaptador Ethernet conectado por cable
echo  Buscando adaptador Ethernet...
echo.

set "ADAPTER="
for /f "tokens=*" %%i in ('powershell -NoProfile -Command "Get-NetAdapter | Where-Object { $_.Status -eq 'Up' -and $_.PhysicalMediaType -notmatch 'Native 802.11|Bluetooth' } | Select-Object -First 1 -ExpandProperty Name"') do (
    set "ADAPTER=%%i"
)
...
if "!ADAPTER!"=="" (
    ...
    echo.
    echo  Verifica que el cable este bien conectado al robot
    echo  y a la PC, luego vuelve a ejecutar este archivo.
    echo.
    pause
    exit /b 1
)
...
REM Verificar si la IP ya esta configurada correctamente
set "YA_CONFIGURADO=0"
for /f "tokens=*" %%i in ('powershell -NoProfile -Command "Get-NetIPAddress -InterfaceAlias '!ADAPTER!' -AddressFamily IPv4 -ErrorAction SilentlyContinue | Select-Object -ExpandProperty IPAddress"') do (
...
REM Configurar IP estatica
echo  Configurando IP estatica 192.168.123.100...

powershell -NoProfile -Command "Get-NetIPAddress -InterfaceAlias '!ADAPTER!' -AddressFamily IPv4 -ErrorAction SilentlyContinue | Remove-NetIPAddress -Confirm:$false -ErrorAction SilentlyContinue" >nul 2>&1
...
powershell -NoProfile -Command "New-NetIPAddress -InterfaceAlias '!ADAPTER!' -IPAddress 192.168.123.100 -PrefixLength 24 -ErrorAction Stop" >nul 2>&1
...
REM Verificar conexion con el robot
:ping_robot
...
if "!APP!"=="" (
    ...
    echo  Asegurate de que el .exe este en la misma carpeta que este .bat
    echo.
    pause
    exit /b 1
)
